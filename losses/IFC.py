...
class IC(nn.Module):
    ...
    def forward(self, v1, v2, y, idx=None):
        ...
        anchor_v2 = torch.index_select(self.memory_v2, 0, y.view(-1)).detach() 
        weight_v2 = torch.index_select(self.memory_v2, 0, idx.view(-1)).detach() 
        weight_v2 = weight_v2.view(batchSize, K + 1, inputSize) 
        weight_v2_relation = weight_v2.unsqueeze(1) - anchor_v2.unsqueeze(1).unsqueeze(0) + 1e-6 
        weight_v2_relation = weight_v2_relation.view(batchSize*batchSize, self.K+1, inputSize) 
        weight_v2_relation = F.normalize(weight_v2_relation, p=2, dim=2)  
        out_v1 = torch.exp(torch.div(weight_v2_relation, T))   
        
        anchor_v1 = torch.index_select(self.memory_v1, 0, y.view(-1)).detach()
