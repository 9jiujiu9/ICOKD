...
from .CRCD import ContrastMemory
from .CRCD import ContrastMemory_queue
...
class CRCDLoss(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.contrast = ContrastMemory(self.feat_dim, self.n_data, self.nce_k, 0.05, 0.5)
...
class GCT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == 'l2':   
            embedding = (x.pow(2).sum((2,3), keepdim=True) + self.epsilon).pow(0.5) * self.alpha   #全局上下文嵌入
            norm = self.gamma / (embedding.pow(2).mean(dim=1, keepdim=True) + self.epsilon).pow(0.5)  #通道归一化
            # norm = self.gamma.pow(0.5) * embedding / (embedding.pow(2).sum(dim=1, keepdim=True) + self.epsilon).pow(0.5)
        ...
        gate = 1. + torch.tanh(embedding * norm + self.beta)   #门控适应
